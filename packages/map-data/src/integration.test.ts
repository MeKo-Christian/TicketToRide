...
function config(n: 2 | 3 | 4 | 5, seed: number): GameConfig {
...
describe('hannoverMap × engine integration', () => {
  ...
  it('initial ticket deck has enough tickets to deal 3 to every player', () => {
    const s = initialState(config(5, 999));
    // 5 players × 3 offered = 15 tickets needed up front; we have 35.
    expect(s.ticketDeck.length + (s.pendingTicketDraw?.offered.length ?? 0)).toBeGreaterThanOrEqual(
      15,
    );
  });
});
